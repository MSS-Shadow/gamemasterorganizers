...
export default function HomePage() {
  const [stats, setStats] = useState({ tournaments: 0, scrims: 0, upcoming: 0, players: 0 });
  ...
  useEffect(() => {
    const fetch = async () => {
      const [tourRes, profileRes, scrimRes, regsRes] = await Promise.all([
      ...
      <div className="relative rounded-lg overflow-hidden border border-border">
        ...
        <div className="absolute bottom-0 left-0 p-6 md:p-10">
          ...
          <p className="text-muted-foreground max-w-xl mb-5 text-sm md:text-base">
            El hogar competitivo de BloodStrike. Torneos, scrims, rankings — todo en un solo lugar.
          </p>
            ...
            {upcomingTournaments.map((t: any) => (
              <div key={t.id} className="bg-card border border-border rounded-lg p-5">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
                  ...
                  <Link to="/tournaments" className="inline-flex items-center gap-2 bg-primary text-primary-foreground px-4 py-2 rounded-md text-sm font-semibold hover:brightness-110 active:scale-95 transition-all self-start">
                    Inscribirse
                  </Link>
                </div>
